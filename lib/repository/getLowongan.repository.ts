import { GetApiResponse, LowonganRow } from "../types/apitypes"
...
export default async function repositoryLowongan (
    param:{
        limit? : number,
        offset? : number,
        query? : string,
...
) : Promise<GetApiResponse<LowonganRow[]>> {
    ...
    const res = await fetch(
        `${baseUrl}/api/list/lowongan?${qs.toString()}`,
        {next:{revalidate:60}}
    )
