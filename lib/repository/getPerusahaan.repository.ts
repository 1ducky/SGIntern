import { GetApiResponse, PerusahaanRow } from "../types/apitypes"
...
export default async function repositoryPerusahaan (
    param:{
        limit? : number,
        offset? : number,
        query? : string,
...
) : Promise<GetApiResponse<PerusahaanRow[]>> {
    ...
    const res = await fetch(
        `${baseUrl}/api/list/perusahaan?${qs.toString()}`,
        {next:{revalidate:60}}
    )
